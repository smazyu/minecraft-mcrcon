...
call :ColorText 07 "===========================================================" && echo.
call :ColorText 07 "PS：可以修改第37行变量指定java版本路径" && echo.
echo. -----------------------------------------------------------------
echo.
echo.                    服务器即将开启,请等待……  
echo.
...
:selectjava
	...
	if %selectjava%==1 (
		...
		echo ======下载Java18——5秒后开始下载======
		ping 127.1 -n 6 >nul 2>nul
		if exist "c:\java18.zip" (
			echo Java18安装脚本已下载。
		) else (
			bitsadmin /transfer Java18下载 /download http://java18.mxxz.work/java18.zip c:\java18.zip
			echo =============
			if exist "c:\java18.zip" (echo 下载完成) else (echo 下载失败)
		)
		echo ======下载unzip.exe——5秒后开始下载======
		ping 127.1 -n 6 >nul 2>nul
		if exist "c:\unzip.exe" (
